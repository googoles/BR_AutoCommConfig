# gui.py
from PyQt5.QtWidgets import QMainWindow, QToolBar, QAction, QFileDialog, QTextBrowser, QStatusBar, QWidget, QVBoxLayout
import os
...
class ExcelImporterGUI(QMainWindow):
    ...
    def import_apj_file(self):
        ...
        if file_dialog.exec_() == QFileDialog.Accepted:
            ...
            # Process the selected APJ file or its directory using apj_handler
            try:
                if os.path.isdir(apj_file_path):
                    # If it's a directory, process it similar to "Import Directory"
                    contents = self.process_physical_folder(apj_file_path)
                    self.display_data(contents)
                else:
                    # If it's a file, process it as an APJ file
